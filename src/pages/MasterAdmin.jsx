...
export default function MasterAdmin() {
  ...
  const handleDeleteFeedback = async (id, name) => {
    if (!window.confirm(`คุณแน่ใจหรือไม่ว่าต้องการลบข้อความของน้อง "${name}"?`)) return;
    try {
      const { error } = await supabase
        .from('feedback')
        .delete()
      ...
      setFeedbacksList(prev => {
        const filtered = prev.filter(fb => fb.id !== id);
      ...
      alert('เกิดข้อผิดพลาดในการลบข้อความ: ' + err.message);
    }
  };

  // ─── Clear all feedbacks ──────────────────────────────────────────────────
  const handleClearAllFeedbacks = async () => {
    if (!window.confirm('⚠️ คำเตือนสำคัญ: คุณต้องการลบข้อความความรู้สึกของน้องๆ ทั้งหมดออกจากระบบใช่หรือไม่? ข้อมูลนี้จะไม่สามารถกู้คืนได้!')) return;
    if (!window.confirm('ยืนยันลบทั้งหมดอีกครั้งจริงหรือไม่? (กดยืนยันรอบสุดท้าย)')) return;
    try {
      const { error } = await supabase
        .from('feedback')
        .delete()
      ...
      alert('ลบข้อความความรู้สึกทั้งหมดเรียบร้อยแล้ว!');
    } catch (err) {
      alert('เกิดข้อผิดพลาดในการลบข้อความทั้งหมด: ' + err.message);
    }
  };
  ...
  useEffect(() => {
    const init = async () => {
      ...
      // 1.5 Load existing feedbacks to populate list and count
      ...
      {showFeedbackQR && (
        <div style={{
          ...
          <div className="glass-card" style={{
            width: '95%', maxWidth: '800px', background: '#1e293b',
            border: '4px solid #ff2e93', boxShadow: '12px 12px 0px #000',
            textAlign: 'center', padding: '1.5rem', color: '#fff',
            ...
            {/* Modal Title */}
            <div style={{ display: 'flex', justifyContent: 'center', gap: '0.6rem', alignItems: 'center', marginBottom: '1.2rem' }}>
              <Heart size={28} color="#ff2e93" fill="#ff2e93" style={{ animation: 'float 2s ease-in-out infinite' }} />
              <h2 style={{ fontSize: '1.6rem', fontWeight: 900, textShadow: '2px 2px 0px #000', margin: 0 }}>
                แผงความรู้สึกจากน้องๆ ค่าย 💖
              </h2>
            </div>
            ...
            <div style={{ display: 'flex', gap: '0.8rem', justifyContent: 'center', marginBottom: '1.5rem' }}>
              ...
              >
                <QrCode size={16} /> QR Code ให้สแกน
              </button>
            </div>
              ...
              <div style={{ animation: 'toastIn 0.2s ease-out both' }}>
                ...
                {fetchingFeedbacks ? (
                  <div style={{ padding: '3rem', color: '#94a3b8', fontWeight: 700 }}>
                    กำลังดึงข้อมูลข้อความความรู้สึก...
                  </div>
                ) : feedbacksList.length === 0 ? (
                  <div style={{
                    background: '#0f172a',
                    ...
                    <span style={{ fontSize: '0.82rem', fontWeight: 600, display: 'block', marginTop: '0.5rem', color: '#64748b' }}>
                      (สแกน QR Code เพื่อร่วมสนุกส่งข้อความมาโชว์หน้าจอนี้ได้ทันทีเรียลไทม์!)
                    </span>
                  </div>
                ) : (
                  <>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
                          ...
                          <div style={{
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: '2.5px dashed #e2e8f0', paddingBottom: '0.8rem', marginBottom: '1rem' }}>
                              ...
                              <button
                                onClick={() => handleDeleteFeedback(currentFb.id, currentFb.student_name)}
                                style={{
                                  background: '#fee2e2',
                                  color: '#ef4444',
                            ...
                            return (
                              ...
                              >
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                  ...
                                  <button
                                    onClick={() => handleDeleteFeedback(fb.id, fb.student_name)}
                                    style={{
                                      background: 'none',
                                      border: 'none',
                                      color: '#cbd5e1',
